...
vector3D recursiveScatter(const ray &r, primitive *world, int depth) {
    hitrecord record;
    if(world->hit(r,0.0001,numeric_limits<float>::max(),record)) {
        ray scatter;
        vector3D attenuation;
        if(depth < SCATTERDEPTH && record.matptr->scatter(r,record,attenuation,scatter)) {
            return attenuation*recursiveScatter(scatter,world,depth+1);
        }
    }
...
vector3D randomInUnitSphere() {
    vector3D point;
    while(true) {
        return point = 2.0*vector3D(randomFloat(),randomFloat(),randomFloat())-vector3D(1,1,1);
