...
TEST_F(vector3Dtest, opPlusTestRandom) {
    +(vr1);
    EXPECT_EQ(x1,vr1.getX());
    EXPECT_EQ(y1,vr1.getY());
    EXPECT_EQ(z1, vr1.getZ());
}
...
TEST_F(vector3Dtest, opAddTestRandom1) {
    vr1 += vr2;
    EXPECT_EQ(x1+x2, vr1.getX());
    EXPECT_EQ(y1+y2, vr1.getY());
    EXPECT_EQ(z1+z2, vr1.getZ());
}
...
TEST_F(vector3Dtest, opAddTestSmall2) {
    float c = 9;
    v1 += c;
    EXPECT_EQ(1+c,v1.getX());
    EXPECT_EQ(2+c,v1.getY());
    EXPECT_EQ(3+c,v1.getZ());
}
TEST_F(vector3Dtest, opAddTestRandom2) {
    float c = dist(gen);
    vr1 += c;
    EXPECT_EQ(x1+c,vr1.getX());
    EXPECT_EQ(y1+c,vr1.getY());
    EXPECT_EQ(z1+c,vr1.getZ());
}
/** Tests operator -() */
TEST_F(vector3Dtest, opMinusTestSmall) {
    vector3D negv1 = -v1;
    EXPECT_EQ(-1, negv1.getX());
    EXPECT_EQ(-2, negv1.getY());
    EXPECT_EQ(-3, negv1.getZ());
}
TEST_F(vector3Dtest, opMinusTestRandom) {
    vector3D negvr1 = -vr1;
    EXPECT_EQ(-x1, negvr1.getX());
    EXPECT_EQ(-y1, negvr1.getY());
    EXPECT_EQ(-z1, negvr1.getZ());
}